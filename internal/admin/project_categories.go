...
func ProjectCategoriesPage(db *gorm.DB) fiber.Handler {
...
<tr style="background:%s;transition:background 0.15s;" onmouseover="this.style.background='#f5f4f5'" onmouseout="this.style.background='%s'">
  ...
  <td style="padding:12px 16px;display:flex;gap:8px;">
    <a href="/admin/project-categories/%s/edit" style="font-size:12px;font-weight:500;color:#1e1d1e;text-decoration:none;padding:4px 10px;background:#f0eff0;border-radius:4px;">Edit</a>
    <button onclick="confirmDelete('/admin/project-categories/%s', '%s')" style="font-size:12px;font-weight:500;color:#b91c1c;background:none;border:none;cursor:pointer;padding:4px 10px;background:#fee2e2;border-radius:4px;">Delete</button>
  </td>
		...
		empty := ""
		if len(categories) == 0 {
			empty = fmt.Sprintf(`<tr><td colspan="4" style="text-align:center;padding:40px;">%s</td></tr>`, emptyState("🏷️", "Belum ada kategori", "Tambahkan kategori untuk project", "New Category", "/admin/project-categories/new"))
		}

		content := fmt.Sprintf(`
%s
<div style="text-align:right;margin-bottom:24px;">
  <a href="/admin/project-categories/new" style="display:inline-flex;align-items:center;gap:6px;padding:8px 18px;background:#1e1d1e;color:#f5f4f5;border-radius:6px;text-decoration:none;font-weight:500;font-size:13px;">
    + New Category
  </a>
</div>
%s
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
  ...
  fetch(_deleteUrl, { method: 'DELETE' }).then(r => { if (r.ok) window.location.href = '/admin/project-categories?flash=deleted'; });
});
</script>`, notifHTML, stats, rows, empty)

		return c.Type("html").SendString(layout("Project Categories", "project_cats", content))
	}
}

// ── Create / Edit Form ────────────────────────────────────────────────────────

func ProjectCategoryFormPage(db *gorm.DB, fallbackID string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		formAction := "/admin/project-categories"
		submitLabel := "Create Category"
		if isEdit {
			title = "Edit Project Category"
			formAction = "/admin/project-categories/" + id
			submitLabel = "Save Changes"
		}

		content := fmt.Sprintf(`
<div style="display:flex; justify-content: flex-end; margin-bottom: 16px;">
  <a href="/admin/project-categories" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;color:#4d4c4d;text-decoration:none;padding:6px 12px;border:1px solid #e5e3e4;border-radius:6px;">← Back to Categories</a>
</div>
<form method="POST" action="%s">
  ...
  <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:16px;padding:16px 0;">
    <a href="/admin/project-categories" style="padding:8px 16px;border:1px solid #e5e3e4;border-radius:6px;color:#4d4c4d;text-decoration:none;">Batal</a>
    <button type="submit" style="padding:8px 20px;background:#1e1d1e;color:#f5f4f5;border:none;border-radius:6px;font-weight:500;">%s</button>
		...
		return c.Type("html").SendString(layout(title, "project_cats", content))
	}
}

// ── Handlers ──────────────────────────────────────────────────────────────────

func CreateProjectCategoryHandler(db *gorm.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		sortOrder := 0
		...
		return c.Redirect("/admin/project-categories?flash=created")
	}
}

func UpdateProjectCategoryHandler(db *gorm.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		id := c.Params("id")
		...
		db.Save(&cat)
		return c.Redirect("/admin/project-categories?flash=updated")
	}
}

func DeleteProjectCategoryHandler(db *gorm.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		id := c.Params("id")
		db.Delete(&model.ProjectCategory{}, "id = ?", id)
